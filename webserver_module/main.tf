...
data "terraform_remote_state" "network" {
  ...
  config = {
    ...
    key    = "environment/dev/network_module/terraform.tfstate" # Path to the state file in the bucket
...
resource "aws_lb_target_group_attachment" "StandaloneWebServers" {
  for_each = {
    az3 = aws_instance.WebServer_AZ3.id
    az4 = aws_instance.WebServer_AZ4.id
  }
